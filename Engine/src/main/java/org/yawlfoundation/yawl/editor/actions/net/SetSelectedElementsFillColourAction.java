...
public class SetSelectedElementsFillColourAction extends YAWLSelectedNetAction
  ...
  {
    putValue(SHORT_DESCRIPTION, " Set the selected elements background colour. ");
    putValue(NAME, "Set Selected Fill Colour...");
    putValue(LONG_DESCRIPTION, "Set the fill colour for the selected elements.");
    putValue(SMALL_ICON, getPNGIcon("color_swatch"));
    putValue(MNEMONIC_KEY, new Integer(java.awt.event.KeyEvent.VK_F));
    putValue(ACCELERATOR_KEY, MenuUtilities.getAcceleratorKeyStroke("F"));
  }
