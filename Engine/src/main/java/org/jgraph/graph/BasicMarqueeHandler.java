...
public class BasicMarqueeHandler {
	...
	/**
	 * Called from mouse dragged to update the marquee org.yawlfoundation.yawl.risk.state during a repaint.
	 */
	...
	/**
	 * Draw the current org.yawlfoundation.yawl.risk.state of the handler. This is called twice by the
	 * overlay method and also by the paint method. The caller's intention is
	 * that the overlay method draws exactly the current org.yawlfoundation.yawl.risk.state of the handler so
	 * that it may be used for XOR paint. The drag method calls overlay, changes
	 * the org.yawlfoundation.yawl.risk.state, and calls overlay again to use this. However, since it is not
