...
public final class YSpecification implements Cloneable, YVerifiable {
    ...
    private YSensorsEditor _sensors;
    ...
    public String toXML() {
        ...
        if(_sensors!=null && _sensors.getNames().size()>0) {
        	xml.append("<sensors notifyTime=\""+_sensors.getGeneralTime()+"\">");
        	for(String nameSensor: _sensors.getNames()) {
        		String sensorTime = _sensors.getTime(nameSensor);
        		if(sensorTime!=null) xml.append("<sensor name=\""+nameSensor+"\" notifyTime=\""+sensorTime+"\">");
        		else xml.append("<sensor name=\""+nameSensor+"\">");
    			xml.append("<vars>");
    			for(String[] var:_sensors.getVariable(nameSensor)) {
    				String name = var[0];
    				String mapping = JDOMUtil.encodeEscapes(var[1]);
    				String type = var[2];
    				if(type!=null) xml.append("<var name=\""+name+"\" mapping=\""+mapping+"\" type=\""+type+"\" />");
    				else xml.append("<var name=\""+name+"\" mapping=\""+mapping+"\" />");
    			}
    			xml.append("</vars>");
    			
    			String riskProbability = JDOMUtil.encodeEscapes(_sensors.getRiskProbability(nameSensor));
    			if(riskProbability != null) {
    				xml.append("<risk>");
    				
	    			xml.append("<riskProbability>"+riskProbability+"</riskProbability>");
	    			
	    			String riskThreshold = JDOMUtil.encodeEscapes(_sensors.getRiskThreshold(nameSensor));
	    			xml.append("<riskThreshold>"+riskThreshold+"</riskThreshold>");
	    			
	    			String riskMessage = JDOMUtil.encodeEscapes(_sensors.getRiskMessage(nameSensor));
	    			if(riskMessage!=null) {
	    				xml.append("<riskMessage>"+riskMessage+"</riskMessage>");
	    			}
	    			
	    			xml.append("</risk>");
    			}
    			
    			String faultCondition = JDOMUtil.encodeEscapes(_sensors.getFaultCondition(nameSensor));
    			if(faultCondition != null) {
    				xml.append("<fault>");
    				
	    			xml.append("<faultCondition>"+faultCondition+"</faultCondition>");
	    			
	    			String faultMessage = JDOMUtil.encodeEscapes(_sensors.getFaultMessage(nameSensor));
	    			if(faultMessage!=null) {
	    				xml.append("<faultMessage>"+faultMessage+"</faultMessage>");
	    			}
	    			
	    			xml.append("</fault>");
    			}
    			
    			String consequence = JDOMUtil.encodeEscapes(_sensors.getConsequence(nameSensor));
    			if(consequence!=null) {
    				xml.append("<consequence>"+consequence+"</consequence>");
    			}
    ...
    public YSensorsEditor getSensors() {
        return _sensors;
    }

    public void setSensors(YSensorsEditor sensors) {
