...
public class ActivityPassTimeInMillis {
	...
	public static Object getActionResult(String WorkDefID, String taskName) {
		...
		if(timeStart != null) {
			if(timeComplete != null) {
				return ""+((new Long(timeComplete.getFirst().getTime()))-(new Long(timeStart.getFirst().getTime())));
			}else {
				return ""+(System.currentTimeMillis()-(new Long(timeStart.getFirst().getTime())));
			}
		}else {
			return null;
		}
	...
	private static List<String> discoveryCouples(LinkedList<ActivityTuple> timeStart, LinkedList<ActivityTuple> timeComplete) {
		...
		for(Entry<Integer, Integer> entry : map.entrySet()) {
			...
			pos = entry.getValue();
			if(pos > -1) {
				result.add(""+(complete[pos]-start[i]));
			}else {
				result.add(""+(System.currentTimeMillis()-start[i]));
			}
		}
		...
		try {
			...
			for(String workDefID : workDefIDs) {
				...
				for(String s : pass) {
					...
					passTime = (long) Double.parseDouble(s);
