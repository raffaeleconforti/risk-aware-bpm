...
public abstract class ForWithMemory {
	...
	public static Object getForResult(LinkedList<Object> input, HashMap<String, String> mappingName, HashMap<String, Object> variables, LinkedList<String> resource, ForInterpreter forInterpreter) {
		...
		Object memory = ForExecutor.inizializeMemory(type); 
		...
		while(!endLoop) {
			...
			if(!endLoop) {
				...
				aggregateResult = (Object[]) ForExecutor.aggregate(res, result, memory, type);
				res = aggregateResult[0];
				memory = aggregateResult[1];
		...
		return ForExecutor.finalize(res, memory, type);
