...
public class AddToVisibleCancellationSetAction extends YAWLSelectedNetAction 
  ...
  {
    putValue(SHORT_DESCRIPTION, " Add selected items to visible cancellation set ");
    putValue(NAME, "Add to Cancellation Set");
    putValue(LONG_DESCRIPTION, " Add selected items to visible cancellation set ");
    putValue(SMALL_ICON, getIconByName("AddToCancellationSet"));
    putValue(MNEMONIC_KEY, new Integer(java.awt.event.KeyEvent.VK_A));
    putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_INSERT,InputEvent.CTRL_MASK));
  }
