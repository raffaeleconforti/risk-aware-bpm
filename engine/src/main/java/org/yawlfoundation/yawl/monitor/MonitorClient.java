...
public class MonitorClient implements Serializable{
    ...
    private String _sensorManagerURI; 
    ...
    private final LinkedList<YSensorNotification> sensorNotificationList = new LinkedList<YSensorNotification>();
    ...
    protected void initInterfaces(Map<String, String> urlMap) {
        ...
        _sensorManagerURI = urlMap.get("sensorService");
        if (_sensorManagerURI == null) _sensorManagerURI = "http://localhost:8080/sensorService/servlet/ss";
    }
    ...
    public void receiveSensorNotification(YSensorNotification ySensorNotification) {
    	lock.lock();
    	sensorNotificationList.addLast(ySensorNotification);
    	lock.unlock();
    }
    
    public LinkedList<YSensorNotification> getSensorNotifications(){
    	lock.lock();
    	LinkedList<YSensorNotification> list = (LinkedList<YSensorNotification>)sensorNotificationList.clone();
    	lock.unlock();
    	return list;
    }
    
    public void removeSensorNotification(YSensorNotification ySensorNotification) {
    	lock.lock();
    	sensorNotificationList.remove(ySensorNotification);
    	lock.unlock();
    }
    
    public String mitigate(String caseID) {
    	lock.lock();
    	String mitigation = MitigationInterface.getInstance(_sensorManagerURI).sendNotification(caseID);
    	lock.unlock();
    	return mitigation;
    }
}